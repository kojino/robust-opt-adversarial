...
parser.add_argument('--args.batch_size', type=int, default=128)
...
for i in range(args.num_noises):
    X_train_adv[i] = np.load(f"../data/{args.data}/adv_{i}.npy")
...
# Attack images
x_deepfool = deepfool.generate(x, **deepfool_params)
...
# Attack images
x_momentum_iterative = momentum_iterative.generate(x, **deepfool_params)
# Consider the attack to be constant
    ...
    for epoch in range(args.epochs):
        ...
        for batch in range(num_batches):
            ...
            for noise_type in range(args.num_noises):
                ...
                loss_total_list[
                    noise_type] += loss_val / (args.epochs * num_batches)
...
logger.info(f"Test Acc Adv: {test_acc_deepfool}")
...
logger.info(f"Test Acc Adv: {test_acc_momentum_iterative}")

# Log experiment result
losses.dump('../result/losses.npy')
weights_distribution.dump('../result/losses.npy')
accs = np.array([test_acc, test_acc_deepfool, test_acc_momentum_iterative])
accs.dump('../result/accs.npy')